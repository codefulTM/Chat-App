...
io.on('connection', (socket) => {
  ...
  socket.on('message_read', async (payload) => {
    ...
    if(onlineMap.has(message?.sender)) {
      const senderSockets = onlineMap.get(message?.sender);
      for(const socketId of senderSockets) {
        io.to(socketId).emit('message_read', {messageId: payload.messageId, conversationId: payload.conversationId});
      }
    }
  });
